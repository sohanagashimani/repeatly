...
export const getApiKeyStatus = async (
  ...
  try {
    ...
    // If status is completed, the key is already ready
    ...
    try {
      ...
      const matchingKey = listResponse.data.keys?.find(key =>
      ...
      // Update the database with the completed key
      // Use upsert to handle potential unique constraint issues
      const updatedApiKey = await prisma.apiKey.update({
        where: { id: apiKey.id },
        data: {
          lastFour: keyString.slice(-4),
          status: "completed",
          // Only update gcpKeyName if it's different from current
      ...
      res.status(200).json({
        ...
        key: keyString, // ⚠️ Return the actual key - show only once!
